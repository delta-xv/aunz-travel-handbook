...
function flightLinks(text){return escapeHtml(text).replace(/\b(MU|JQ|VA)(\d{2,4})\b/g,(flight,prefix,number)=>`<a class="flight-link" href="https://www.flightaware.com/live/flight/${{MU:'CES',JQ:'JST',VA:'VOZ'}[prefix]}${number}" target="_blank" rel="noopener noreferrer" aria-label="${flight} 航班动态（FlightAware）">${flight} ↗</a>`)}
...
function syncNavigation(){const route=routeState(),sheetState=history.state?.app==='aunz-touch'&&['places','map','share','copy'].includes(history.state.sheet?.type)?history.state.sheet:null;const changed=activeView!==route.view,changedDay=expandedDay!==route.day;let returnFocus=null;
