...
function flightQueryInfo(flight){
  const match=/^(MU|JQ|VA)(\d{2,4})$/.exec(flight),date=tripFlightDates[flight];
  if(!match||!date)return null;
  const airline=match[1]==='JQ'?{name:'捷星',url:'https://booking.jetstar.com/flight-status/?culture=zh-cn'}:match[1]==='MU'?{name:'东航',url:'https://www.ceair.com/zh/usd/self-service/before/flight-status'}:null;
  return {flight,date,airline,backup:'https://www.flightaware.com/live/flight/'+{MU:'CES',JQ:'JST',VA:'VOZ'}[match[1]]+match[2]};
}
function flightLinks(text){return escapeHtml(text).replace(/\b(MU|JQ|VA)(\d{2,4})\b/g,flight=>`<button type="button" class="flight-link" data-flight="${flight}" aria-haspopup="dialog" aria-label="查询 ${flight} 航班动态">${flight} ⌄</button>`)}
...
function renderSheet(data){const title=document.getElementById('sheetTitle'),content=document.getElementById('sheetContent');content.scrollTop=0;document.getElementById('sheetStatus').textContent='';
  if(data.type==='flight'){
    const info=flightQueryInfo(data.flight);title.textContent='航班查询';
    if(!info){content.textContent='未找到航班信息';return}
    content.innerHTML=`<div class="flight-query-fields"><label for="flightNumber">航班号</label><div><input id="flightNumber" readonly value="${info.flight}"><button id="copyFlightNumber" class="outline-btn">复制航班号</button></div><label for="flightDate">出发日期（当地）</label><div><input id="flightDate" readonly value="${info.date}"><button id="copyFlightDate" class="outline-btn">复制日期</button></div></div><div class="flight-query-options">${info.airline?`<a class="primary-btn" data-flight-provider="official" href="${escapeHtml(info.airline.url)}" target="_blank" rel="noopener noreferrer">${info.airline.name}官网（中文）↗</a>`:''}<a class="outline-btn" data-flight-provider="backup" href="${info.backup}" target="_blank" rel="noopener noreferrer">FlightAware 备用查询 ↗</a></div><p class="source-note">官网需手动输入航班号和日期；备用站也请核对日期。</p>`;
    document.getElementById('copyFlightNumber').onclick=()=>copyText(info.flight,document.getElementById('flightNumber'));
    document.getElementById('copyFlightDate').onclick=()=>copyText(info.date,document.getElementById('flightDate'));
    return;
  }
...
document.addEventListener('click',e=>{const button=e.target.closest('[data-flight]');if(button)openSheet({type:'flight',flight:button.dataset.flight},button)});
