...
function syncNavigation(){let returnFocus=null;const route=routeState(),sheetState=history.state?.app==='aunz-touch'?history.state.sheet:null;const changed=activeView!==route.view;
  if(changed){if(activeView)viewScroll[activeView]=scrollY;activeView=route.view;document.querySelectorAll('.view').forEach(v=>v.hidden=v.id!==activeView);document.querySelectorAll('[data-nav]').forEach(a=>{const active=a.hash==='#'+activeView;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current')});requestAnimationFrame(()=>{if(activeView==='itinerary'&&route.day===null)scrollToCurrentDay();else window.scrollTo(0,viewScroll[activeView]||0)})}
...
function goView(id){if(activeView===id&&!isOpen(dialog)){if(id==='itinerary'){scrollToCurrentDay();return}window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});return}history.pushState(null,'','#'+id);syncNavigation()}
...
function scrollToCurrentDay(){if(activeView!=='itinerary'||isOpen(dialog)||isOpen(sheet))return;setItineraryFilter('all');const today=currentDayIndex(),index=today>=0?today:dateKey(new Date())<tripDates[0]?0:itinerary.length-1;const row=document.getElementById('day-'+(index+1)),header=document.getElementById('itineraryHeader');const top=scrollY+row.getBoundingClientRect().top-header.getBoundingClientRect().height-12;window.scrollTo({top:Math.max(0,top),behavior:'auto'})}
document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{setItineraryFilter(b.dataset.filter);const header=document.getElementById('itineraryHeader');window.scrollTo({top:scrollY+header.getBoundingClientRect().top,behavior:'auto'})});
