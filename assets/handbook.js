...
function placeCopyName(index,name){return placeName(name)+'，'+(index===0||index===14?'中国':index<7?'澳大利亚':'新西兰')}
...
function placeButtons(x,heading=true){const index=Number(x.d.slice(1))-1;return `${heading?'<div class="places-label">地点与路线</div>':''}<div class="places">${dayPlaces(index).map((name,j)=>`<button class="place place-button" data-place-day="${index}" data-place-index="${j}" aria-haspopup="dialog"><span>⌖ ${escapeHtml(placeName(name))}</span><span aria-hidden="true">›</span></button>`).join('')}</div>`}
...
function renderSheet(data){const title=document.getElementById('sheetTitle'),content=document.getElementById('sheetContent');content.scrollTop=0;document.getElementById('sheetStatus').textContent='';
  if(data.type==='map'){const name=dayPlaces(data.day)[data.place];if(!name){content.textContent='地点不存在';return}title.textContent='用地图打开';const order=isAppleMobile?['apple','amap','baidu','google']:['amap','baidu','google','apple'];content.innerHTML=`<p class="place-name">${escapeHtml(placeName(name))}</p><div class="provider-list">${order.map(provider=>`<div class="provider-row"><strong>${mapNames[provider]}</strong><div><a class="outline-btn" href="${escapeHtml(mapURL(provider,data.day,name))}" target="_blank" rel="noopener">查看地点 ↗</a>${['apple','google'].includes(provider)?`<a class="outline-btn" href="${escapeHtml(mapURL(provider,data.day,name,true))}" target="_blank" rel="noopener">路线 ↗</a>`:''}</div></div>`).join('')}</div><label class="copy-label" for="copyField">地点名称</label><textarea id="copyField" readonly rows="2">${escapeHtml(placeCopyName(data.day,name))}</textarea><button class="outline-btn sheet-copy" id="copySheet">复制地点名称</button>`;
