...
function flightLinks(text){return escapeHtml(text).replace(/\b(MU|JQ|VA)(\d{2,4})\b/g,(flight,prefix,num)=>`<a class="flight-link" href="https://www.flightaware.com/live/flight/${{MU:'CES',JQ:'JST',VA:'VOZ'}[prefix]}${num}" target="_blank" rel="noopener" title="查看 ${flight} 航班动态">${flight} ↗</a>`)}
...
itinerary.forEach((x,i)=>{const el=document.createElement('article');el.id='day-'+(i+1);el.className='day card';el.dataset.region=i===0||i===14?'flight':i<7?'au':'nz';el.innerHTML=`<button class="day-head" aria-haspopup="dialog"><span class="day-index"><b>${x.d}</b><small>${x.date}</small></span><span class="day-copy"><span class="day-title">${x.title}</span><span class="day-caption">${x.time}</span></span>${logbookIcon('arrow')}</button><div class="day-body">${detailMarkup(x)}</div>`;el.querySelector('button').onclick=e=>openDay(i,e.currentTarget);document.getElementById('timeline').appendChild(el)});
const dialog=document.getElementById('dayDialog'),sheet=document.getElementById('actionSheet');
let dialogIndex=0,dialogOpener=null,sheetOpener=null,activeView='',renderedSheet='',lastDialogIndex=-1;
const viewIds=['today','itinerary','route-map','confirmed','tips'],viewScroll={};
...
function routeState(){const parts=location.hash.slice(1).split('/'),view=viewIds.includes(parts[0])?parts[0]:'today',match=/^day-(\d+)$/.exec(parts[1]||'');return{view,day:match&&Number(match[1])>=1&&Number(match[1])<=15?Number(match[1])-1:null}}
function renderDialog(){const x=itinerary[dialogIndex];document.getElementById('dialogDate').textContent=`${x.d} · 2026/${x.date}`;document.getElementById('dialogTitle').textContent=x.title;const content=document.getElementById('dialogContent');content.innerHTML=detailMarkup(x);content.scrollTop=0;document.getElementById('prevDay').disabled=dialogIndex===0;document.getElementById('nextDay').disabled=dialogIndex===14;document.getElementById('dialogPosition').textContent=`${dialogIndex+1} / 15`;lastDialogIndex=dialogIndex}
function syncNavigation(){let returnFocus=null;const route=routeState(),sheetState=history.state?.app==='aunz-touch'?history.state.sheet:null;const changed=activeView!==route.view;
  if(changed){if(activeView)viewScroll[activeView]=scrollY;activeView=route.view;document.querySelectorAll('.view').forEach(v=>v.hidden=v.id!==activeView);document.querySelectorAll('[data-nav]').forEach(a=>{const active=a.hash==='#'+activeView;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current')});requestAnimationFrame(()=>window.scrollTo(0,viewScroll[activeView]||0))}
  if(route.day!==null){dialogIndex=route.day;if(lastDialogIndex!==dialogIndex)renderDialog();const wasOpen=isOpen(dialog);openModal(dialog);if(!wasOpen)focusQuietly(document.getElementById('closeDay'))}else if(isOpen(dialog)){closeModal(dialog);lastDialogIndex=-1;returnFocus=dialogOpener}
  ...
  document.body.classList.toggle('dialog-open',isOpen(dialog)||isOpen(sheet));document.querySelector('.app').inert=isOpen(dialog)||isOpen(sheet);document.querySelector('.mobile-nav').inert=isOpen(dialog)||isOpen(sheet);dialog.inert=isOpen(sheet);document.getElementById('modalBackdrop').hidden=![dialog,sheet].some(e=>isOpen(e)&&e.classList.contains('fallback-dialog'));
...
function openDay(i,opener=document.activeElement){if(i<0||i>14)return;dialogOpener=opener;history.pushState({app:'aunz-touch',kind:'day'},'','#'+routeState().view+'/day-'+(i+1));syncNavigation()}
function changeDay(step){const next=dialogIndex+step;if(next<0||next>14)return;history.replaceState(history.state,'','#'+routeState().view+'/day-'+(next+1));syncNavigation()}
function closeDay(){if(history.state?.app==='aunz-touch'&&history.state.kind==='day'){history.back()}else{history.replaceState(null,'','#'+routeState().view);syncNavigation()}}
...
function goView(id){if(activeView===id&&!isOpen(dialog)){window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});return}history.pushState(null,'','#'+id);syncNavigation()}
...
document.getElementById('closeDay').onclick=closeDay;document.getElementById('closeDayBottom').onclick=closeDay;dialog.addEventListener('cancel',e=>{e.preventDefault();closeDay()});document.getElementById('closeSheet').onclick=closeSheet;sheet.addEventListener('cancel',e=>{e.preventDefault();closeSheet()});document.getElementById('prevDay').onclick=()=>changeDay(-1);document.getElementById('nextDay').onclick=()=>changeDay(1);
[dialog,sheet].forEach(el=>el.addEventListener('click',e=>{if(e.target!==el)return;const r=el.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)(el===sheet?closeSheet:closeDay)()}));
document.getElementById('modalBackdrop').onclick=()=>isOpen(sheet)?closeSheet():closeDay();
...
document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));document.querySelectorAll('.day').forEach(d=>d.hidden=b.dataset.filter!=='all'&&d.dataset.region!==b.dataset.filter)});
document.addEventListener('keydown',e=>{const top=isOpen(sheet)?sheet:isOpen(dialog)?dialog:null;if(!top?.classList.contains('fallback-dialog'))return;if(e.key==='Escape'){e.preventDefault();top===sheet?closeSheet():closeDay()}if(e.key==='Tab'){const controls=[...top.querySelectorAll('a[href],button:not([disabled]),textarea,input,select,summary,[tabindex="0"]')].filter(el=>el.getClientRects().length);if(!controls.length)return;const first=controls[0],last=controls.at(-1);if(e.shiftKey&&(document.activeElement===first||!top.contains(document.activeElement))){e.preventDefault();last.focus()}else if(!e.shiftKey&&(document.activeElement===last||!top.contains(document.activeElement))){e.preventDefault();first.focus()}}});
let swipe=null;
const swipeContent=document.getElementById('dialogContent');
swipeContent.addEventListener('pointerdown',e=>{if(e.pointerType!=='touch'||!e.isPrimary||e.clientX<28||e.clientX>innerWidth-28||e.target.closest('a,button,summary,input,select,textarea'))return;swipe={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now()}});
swipeContent.addEventListener('pointercancel',()=>swipe=null);
swipeContent.addEventListener('pointerup',e=>{if(!swipe||e.pointerId!==swipe.id)return;const dx=e.clientX-swipe.x,dy=e.clientY-swipe.y,elapsed=performance.now()-swipe.t;swipe=null;if(Math.abs(dx)>85&&Math.abs(dy)<45&&elapsed<900)changeDay(dx<0?1:-1)});
...
document.getElementById('todayPlaces').onclick=e=>openSheet({type:'places',day:displayedDay},e.currentTarget);document.getElementById('shareToday').onclick=e=>shareDay(displayedDay,e.currentTarget);document.getElementById('shareDay').onclick=e=>shareDay(dialogIndex,e.currentTarget);
...
function checkDate(){const key=dateKey(new Date());if(key!==lastSystemDate){lastSystemDate=key;renderToday()}else{updateNextEvent();if(Date.now()-lastWeatherRefresh>=1800000)loadWeather()}}
...
document.querySelectorAll('#confirmed .compact-row').forEach(row=>{const span=row.lastElementChild;span.innerHTML=span.innerHTML.replace(/\b(MU|JQ|VA)(\d{2,4})\b/g,flight=>flightLinks(flight));const route=row.children[1];route.innerHTML=route.textContent.split(' → ').map(n=>wikiTitles[n]?wikiLink(n,true):escapeHtml(n)).join(' → ')});


syncNavigation();
window.addEventListener('load',()=>requestAnimationFrame(()=>{if(!isOpen(dialog)&&!isOpen(sheet))window.scrollTo(0,viewScroll[activeView]||0)}));
