...
function renderSheet(data){window.routeMap.clearPlace();const title=document.getElementById('sheetTitle'),content=document.getElementById('sheetContent');content.scrollTop=0;document.getElementById('sheetStatus').textContent='';
  if(data.type==='map'){const name=dayPlaces(data.day)[data.place];if(!name){content.textContent='地点不存在';return}title.textContent=placeName(name);content.innerHTML=`<div id="placeMapCanvas" class="interactive-map place-map" role="region" aria-label="地点地图，可拖动和缩放"></div><div class="place-map-status"><span id="placeMapStatus" role="status"></span><button id="retryPlaceMap" class="outline-btn" hidden>重试</button></div><div class="place-copy"><div class="place-copy-row"><textarea id="copyPlaceZh" readonly rows="2" aria-label="中文地点名称">${escapeHtml(placeCopyName(name))}</textarea><button class="outline-btn" data-copy-field="copyPlaceZh">复制中文</button></div><div class="place-copy-row"><textarea id="copyPlaceEn" readonly rows="2" lang="en" aria-label="英文地点名称">${escapeHtml(placeCopyName(name,'en'))}</textarea><button class="outline-btn" data-copy-field="copyPlaceEn">复制英文</button></div></div>`;
  ...
  content.querySelectorAll('[data-copy-field]').forEach(button=>button.onclick=()=>{const field=document.getElementById(button.dataset.copyField);copyText(field.value,field)});
