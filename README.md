...
点击航班号打开查询面板，显示航班号和出发地当地日期，支持分别复制。优先提供东航或捷星的中文官方查询页，并保留 FlightAware 备用入口。官网需要手动输入查询条件；备用站可能默认显示最近一班，需要核对日期。外部站点的风控、语言和查询日期范围由服务方决定。
...
页眉提供唯一的“出团通知”入口。打开后从第一页阅读完整 HTML 原文，仅保留上一页、下一页。文字、航班表、保险和费用表均按原稿转换；原稿中的入境物品图已逐项转为文字。原 PDF 仅作为转换源保留，页面不提供下载入口，也不加载任何图片或 PDF。
