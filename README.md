...
- **今日**：顶部仅显示当天日期，字号 13px；按设备日期显示当天行程。出发前显示 D1，结束后显示返程信息。横滑日期条可预览其他天，点击“今天”恢复自动选择。当天地点以中英双语列表默认展开，可手动收起。穿衣与随身物品默认展开，同样可收起；内容面板使用统一的深蓝底色与浅色文字。
...
航班号使用紧凑的行内标签；航班、百科、地点和景点展开入口统一使用导航深蓝底色与浅色文字，城市链接保留下划线；外部链接标记 ↗，页内展开项使用上下箭头，并提供按下反馈。右上角“出团通知”为与导航深蓝色一致的独立入口。
