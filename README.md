...
- **地图**：按设备日期选择默认区域：D2–D7（9月25–30日）为澳大利亚，D8–D14（10月1–7日）为新西兰；D1、D15 及行程日期之外为全程。同一天内保留手动切换的区域，日期变化后再次进入地图会重新匹配。使用 Leaflet + OpenStreetMap 交互地图，可切换澳大利亚／新西兰，拖动、双指缩放，点击地点查看关联行程。“全程”显示上海往返澳新两国的完整路线，再次点击可关闭地点弹窗并恢复全程视野。澳大利亚／新西兰按钮聚焦对应国家，再次点击恢复该国视野。连线表示行程顺序，不代表实际道路或航线。

点击航班号直接打开 FlightAware 对应航班页面。页面可能默认显示最近一班，请核对出行日期。

航班号使用紧凑的行内标签；航班、百科和景点展开入口使用导航深蓝底色与浅色文字，地点列表使用透明底色与描边，城市链接保留下划线；外部链接标记 ↗，页内展开项使用上下箭头，并提供按下反馈。右上角“出团通知”为与导航深蓝色一致的独立入口。

上海显示为普通文字，其他城市与景点提供中文 Wikipedia 链接。没有对应词条时，提供百科搜索或标明相关词条。
